import java.io.*;
import java.util.*;
public class USACOJanuary {
    static FastIO r = new FastIO();
    static PrintWriter pr = new PrintWriter(new BufferedWriter(new OutputStreamWriter(System.out)));

    public static void main(String[] args) throws Exception {
        String[] guess = new String[3], correct = new String[3];
        int yellowCount = 0, greenCount = 0;
        for (int i = 0; i  < 3; i++) correct[i] = r.readLine();
        for (int i = 0; i < 3; i++) guess[i] = r.readLine();
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (guess[i].charAt(j) == correct[i].charAt(j)) {
                    greenCount++;
                }
            }
        }
