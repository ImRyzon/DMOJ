...
public class NetWeight {
    ...
    public static void main(String[] args) throws Exception {
        int n = r.readInt(), arr[] = new int[n];
        for (int i = 0; i < n; i++) {int p = r.readInt();if (p <= 100) arr[i] = p;}
        Arrays.sort(arr);
        if (n == 1) pr.println(arr[0]);
        else pr.println(arr[arr.length-1] + arr[arr.length-2]);
        pr.flush();
    }
