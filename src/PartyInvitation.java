...
public class PartyInvitation {
    ...
    public static void main(String[] args) throws Exception {
        int k = r.readInt(), m = r.readInt(), nums[] = new int[k];
        for (int i = 0; i < m; i++) {
            int num = r.readInt(), index = 0;
            for (int j = 0; j < nums.length; j++) {
                if (nums[j] == 1) continue;
                index++; if (index % num == 0) {nums[j] = 1; index = 0;}
            }
        }
        for (int i = 0; i < nums.length; i++) {if (nums[i] == 1) continue; pr.println(i+1);}
        pr.flush();
    }
