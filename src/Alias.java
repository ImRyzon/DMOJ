...
public class Alias {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<String> words = new ArrayList<>(); words.add("");
        List<Edge> adj[] = new ArrayList[n+1];
        for (int i = 1; i <= n; i++) adj[i] = new ArrayList<>();
        for (int i = 1; i <= m; i++) {
            String a = r.next(), b = r.next(); int w = r.readInt();
            if (!words.contains(a)) words.add(a);
            if (!words.contains(b)) words.add(b);
            adj[words.indexOf(a)].add(new Edge(words.indexOf(b), w));
        }
        int q = r.readInt();
        for (int i = 1; i <= q; i++) {
            String a = r.next(), b = r.next();
            int dis[] = new int[n+1];
            dijkstra(words.indexOf(a), adj, dis);
            pr.println(dis[words.indexOf(b)] == Integer.MAX_VALUE ? "Roger" : dis[words.indexOf(b)]);
        }
        pr.close();
    }
