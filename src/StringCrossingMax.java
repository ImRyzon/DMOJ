import java.io.*;
import java.util.*;
public class StringCrossingMax {
    static FastIO r = new FastIO();
    static PrintWriter pr = new PrintWriter(new BufferedWriter(new OutputStreamWriter(System.out)));

    public static void main(String[] args) throws Exception {
        int a = r.readInt(), b = r.readInt(), counter = 1000001, ans = 0;
        String s = r.readLine(), t = r.readLine(), mostOccur = "";

        for (int i = 0; i < a; i++) {
            if (s.replaceAll(String.valueOf(s.charAt(i)), "").length() < counter) {
                counter = s.replace(String.valueOf(s.charAt(i)), "").length();
                mostOccur = String.valueOf(s.charAt(i));
            }
        }

        for (int i = 0; i < b; i++) {
            if (s.replaceAll(String.valueOf(t.charAt(i)), "").length() > counter) {
