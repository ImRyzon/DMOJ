...
public class AlpacaRacing {
    ...
    public static void main(String[] args) throws IOException {
        int n = r.readInt(), d = r.readInt(), k = r.readInt(), x = r.readInt(), p, index = 0;
        Integer[] speeds = new Integer[n];
        for (int i = 0; i < n; i++) speeds[i] = r.readInt();
        Arrays.sort(speeds, Collections.reverseOrder()); p = r.readInt();
        boolean canWin = false;
        for (int i = 0; i < k; i++) {
            speeds[index] = (speeds[index]*((100-x)/100));
            if (speeds[index] < p) index++;
            if (index == speeds.length || speeds[index] < p) {canWin = true; break;}
        }
        if (canWin) pr.println("YES"); else pr.println("NO");
        pr.flush();
    }
